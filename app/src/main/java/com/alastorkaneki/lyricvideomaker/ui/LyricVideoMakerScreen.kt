...
fun LyricVideoMakerScreen(amoled: Boolean, onAmoledChanged: (Boolean) -> Unit) {
    ...
    var selectedModel by remember { mutableStateOf(WhisperModel.TINY_MULTILINGUAL) }
    ...
    val modelInstalled = remember(selectedModel, modelRevision) { modelManager.isInstalled(selectedModel) }
    ...
    val outputPicker = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("video/mp4")) { uri ->
        ...
        if (uri != null && selectedAudio != null && artwork != null && size != null && timedLyrics != null) scope.launch {
            busy = "Rendering MP4"
            renderProgress = 0
        ...
        LazyColumn(
            ...
            item {
                Section("2. Artwork and automatic canvas") {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        OutlinedButton(onClick = { imagePicker.launch(arrayOf("image/*")) }) { Text("Replace image") }
                    ...
                    Text("The output follows the image aspect ratio. Brightness stays unchanged unless dimming is raised.")
                }
            }
            ...
            item {
                Section("3. Offline transcription model") {
                    ...
                    Text(if (modelInstalled) "Model installed and ready." else "Download this model once; it works offline afterward.")
                        ...
                        Button(
                            onClick = {
                                scope.launch {
                                    ...
                                    runCatching {
                                        modelManager.download(selectedModel) { modelDownloadProgress = it }
                                    }.onSuccess {
                                        modelRevision++
                                        status = "${selectedModel.displayName} is installed. Transcription is now offline."
                                    }.onFailure {
            ...
            item {
                Section("4. Lyrics and timing") {
                    OutlinedButton(onClick = { lyricPicker.launch(arrayOf("text/*", "application/octet-stream")) }) {
                        Text("Import TXT/LRC")
                    }
                    OutlinedTextField(
                        value = lyrics,
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button(
                            onClick = {
                                val selectedAudio = audio ?: return@Button
                                scope.launch {
                                    busy = "Transcribing locally"
                                    runCatching {
                                        transcriber.transcribe(selectedAudio.uri, modelManager.modelFile(selectedModel))
                                    }.onSuccess { result ->
                                        ...
                                        status = "Offline transcription complete. Aligned ${alignment?.matchedWords}/${alignment?.totalReferenceWords} words."
                                    }.onFailure {
                                        status = it.message ?: "Offline transcription failed."
                                    }
                                    busy = null
                                }
                            },
                            enabled = audio != null && modelInstalled && busy == null,
                        ) { Text("Transcribe offline + align") }

                        OutlinedButton(
                            onClick = {
                                val selectedAudio = audio ?: return@OutlinedButton
                                runCatching {
                                    LyricsAlignmentEngine.align(lyrics, transcription, selectedAudio.durationMs, lyricsSource)
                                }.onSuccess {
                                    alignment = it
                            ...
                            enabled = audio != null && lyrics.isNotBlank(),
                        ) { Text("Use existing timing") }
                    }
                    alignment?.let {
                        Text("${it.lines.size} lines • ${(it.confidence * 100).toInt()}% alignment confidence")
                    }
                }
            }
            ...
            item {
                Section("6. Style and export") {
                    ...
                    Slider(value = fontScale, onValueChange = { fontScale = it }, valueRange = 0.7f..1.6f)
                    Text("Background dim ${(dim * 100).toInt()}%")
                    Slider(value = dim, onValueChange = { dim = it }, valueRange = 0f..0.75f)
                    ...
                    if (busy != null) {
                        Row(
                            ...
                            Text("$busy${if (busy == "Rendering MP4") " • $renderProgress%" else ""}")
                        }
                    }
                }
            }
